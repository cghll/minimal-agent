...
我认为两者解决的是不同问题：
- Context 管理决定“哪些信息应该交给模型”，直接影响 Agent 效果。
- KV Cache复用已经计算过的上下文前缀，降低 Prefill 时间和推理成本。
因此，综合最优不是一味缩短 Context，也不是单纯追求缓存命中率，而是：
在保留任务目标、关键证据和工具调用链的前提下，让上下文尽可能短，并让稳定内容形成可复用前缀。

具体可以把 Context 分成三层：
稳定前缀
Agent 指令、安全规则、工具 Schema、输出格式

半稳定上下文
当前任务、已确认事实、阶段性结论、关键错误

动态后缀
当前问题、最新工具结果、实时环境、检索结果
稳定内容放在前面，并保证内容、顺序和序列化格式不变；动态内容放在最后。这样每次 Agent 调用虽然尾部发生变化，模型仍然可以复用前面的 KV Cache。
Context 不能简单采用“最近 N 轮”，而应按价值进行选择：
- 必须保留当前任务、安全约束和未完成事项；
- 完整保留 tool_call + tool_result，避免只看到调用看不到结果；
- 保留关键错误、用户纠偏和最终结论；
- 压缩重复解释、过期状态和大段无效日志；
- 保留最近几轮原文，较早历史压缩为 checkpoint。

**HarbSSH 案例**
以 HarbSSH 的 SSH 故障诊断 Agent 为例，用户要求“排查 Nginx 502”。
Agent 的稳定前缀包括 SSH Agent 指令、安全规则和工具定义；这些内容每轮基本不变，适合复用 KV Cache。
执行过程中产生的内容则分层管理：
稳定前缀：
SSH Agent 指令 + 安全规则 + 工具 Schema

任务上下文：
排查 Nginx 502 + 已确认的服务器环境 + 当前排查阶段

关键历史：
用户纠偏 + permission denied 等错误 + 已验证结论

动态后缀：
本轮问题 + 最新 SSH 命令结果 + 当前终端状态
HarbSSH 使用 HybridReducer 同时保留高价值消息和最近对话；通过 TaskProvider 保存当前任务，MilestoneTracker 保存关键错误和用户纠偏，ToolResultProvider 将过长的命令输出压缩为摘要，LongTermMemoryProvider 只召回与当前任务相关的历史信息。
例如工具返回数千行 Nginx 日志时，不应该把完整日志在后续每一轮重复放入 Context，而是保留：
关键错误：upstream timed out
涉及地址：127.0.0.1:8080
发生时间：10:32
原始结果引用：tool-result-xxx
下一轮只追加新的工具证据。这样既避免关键事实丢失，也减少输入 Token；同时前面的 Agent 指令、工具定义和既有历史保持不变，可以继续命中前缀缓存。
如何判断综合最优
不能只看 KV Cache 命中率，还要联合观察：
缓存命中率 = cached_tokens / prompt_tokens
性能：TTFT、P95/P99 延迟、输入 Token 数
效果：任务成功率、工具调用次数、重复调用率、错误恢复率
如果缓存命中率提高，但关键错误被压缩掉，导致 Agent 重复执行命令，就不是真正的优化。
最终原则是：
稳定内容前置以提高 KV Cache 命中率，动态内容后置以控制失效范围；关键事实结构化保留，低价值历史及时压缩；用任务成功率和端到端延迟共同评价，而不是只优化某一个指标。
